...
describe("PublicReportsMapV2 marker grouping", () => {
	beforeEach(() => {
	...
	it("zooms into a separable overlap before opening the report stack", async () => {
		...
		await user.click(
			screen.getByRole("button", { name: "2 reports in this area" }),
		);
	...
	it("opens exact and max-zoom stacks immediately", async () => {
		...
		await user.click(
			screen.getByRole("button", {
				name: "2 reports at this public location",
			}),
		);
