...
Scope note: deployment acceptance covers the resident and public experience. Prototype officer routes remain in the repository, but they are out of scope for the assessed implementation. The current deployed public edge uses Amazon CloudFront at `d2yol17g6mes38.cloudfront.net`; its origin and cache settings are managed outside this repository.
...
```
Users → CloudFront edge → EC2 origin (Nginx + Frontend + Backend + YOLO) → RDS (PostgreSQL + PostGIS)
                                                    ↓
                                              S3 (optional backup)
```
...
- **CloudFront:** Current public edge entrypoint; configuration is external to this repo and must be verified in AWS Console
...
- Frontend: CloudFront URL, custom domain, or `http://<EC2_IP>` if testing origin directly
- Backend health: `http://<EC2_IP>/api/health`
- Submit test report
- Check public map and status flow
...
| **S3 + CloudFront + Lambda/API Gateway** | Full split deployment complexity, Lambda package size limits  |
...
- Experimental officer dashboard accessible
