...
Updated on 2026-05-13 after Stage 1 Kaggle output consolidation.
...
| Current YOLO model | `ml_workspace/models/current_yolo/best.pt` | 6 MB | Current model integrated by the backend. Confirmed SHA-256 is `215b16ea72f450839966b22e2d17e342d40bf0cd3c6becb38b048dc21eb888e7`. Model class names are `artificial_container`, `drain_inlet`, and `tire`. |
...
The current `best.pt` model is a retained three-class checkpoint aligned to `artificial_container`, `drain_inlet`, and `tire`. A local source tree exists outside this workspace and is linked for reference, while the proper v1 training path should use the Kaggle dataset directly and export only the resulting model and metrics back into `ml_workspace`.
